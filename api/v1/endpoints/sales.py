...
from crud.sales.crud_sale_item import sale_item
...
@sales_router.delete("/{sale_id}", response_model=Sale)
async def delete_sale(
    *,
    ...
    """Delete a sale and its associated items"""
    db_sale = await sale.get(db, id=sale_id)
    if not db_sale or db_sale.tenant_id != current_user.tenant_id:
        raise HTTPException(status_code=404, detail="Sale not found")
    ...
    # Delete all associated sale items first
    try:
        await sale_item.remove_by_sale(db, sale_id=sale_id)
